...
from jax_data import TrainDatasetNaive, TrainDatasetContrastive
from utils.code_mix import get_codemixed_ids, get_dict
from torch.utils.data import DataLoader, IterableDataset
from tqdm import tqdm
from transformers import AutoConfig, AutoTokenizer, FlaxAutoModel, BasicTokenizer
...
from tevax.training import TiedParams, RetrieverTrainState, grad_cache_train_step, \
    DualParams, retriever_train_step_contrastive, retriever_train_step_naive
...
def main():
    ...
    if data_args.codemix_ratio > 0:
        logger.info(f"overwriting codemix_ratios to {data_args.codemix_ratio}")
        data_args.codemix_ratio_query = data_args.codemix_ratio
        data_args.codemix_ratio_document = data_args.codemix_ratio
    if data_args.codemix_sentence_ratio > 0:
        logger.info(f"overwriting codemix_sentence_ratios to {data_args.codemix_sentence_ratio}")
        data_args.codemix_sentence_ratio_query = data_args.codemix_sentence_ratio
        data_args.codemix_sentence_ratio_document = data_args.codemix_sentence_ratio
    if data_args.cm_loss_weight > 0:
        logger.info(f"overwriting cm_loss_weights to {data_args.cm_loss_weight}")
    ...
    def tokenize_train(example):
        if training_args.contrastive and not data_args.codemix_in_runtime:
        ...
        if (
            ...
            and (
                data_args.codemix_ratio_query > 0
                or data_args.codemix_ratio_document > 0
            )
                ...
                dict(
                    {
                        "input_ids": get_codemixed_ids(
                            ...
                            x,
                            data_args.p_max_len,
                            data_args.codemix_ratio_document,
                        )
                    }
                )
                for x in pos_psgs
    ...
    train_data = train_data.filter(
        function=lambda data: len(data["pos_psgs_input_ids"]) >= 1 and \
                              len(data["neg_psgs_input_ids"]) >= data_args.train_n_passages-1, num_proc=data_args.dataset_proc_num
    )

    TrainDataset = TrainDatasetContrastive if training_args.contrastive else TrainDatasetNaive
    train_dataset = TrainDataset(train_data, data_args, tokenizer, basic_tokenizer, src2tgt)
    ...
    def get_cm_target():
        if (
            data_args.codemix_ratio == 0
            and data_args.codemix_ratio_query == 0
            and data_args.codemix_ratio_document == 0
        ):
            return None
        elif data_args.codemix_ratio_query > 0 and data_args.codemix_ratio_document == 0:
            return "q"
        elif data_args.codemix_ratio_query == 0 and data_args.codemix_ratio_document > 0:
            return "p"
        else:
    ...
    if training_args.grad_cache:
        ...
        if training_args.contrastive:
            def retriever_train_step_wrapper(*inputs):
                return retriever_train_step_contrastive(
                    *inputs,
                    cm_target=cm_target,
                    cm_loss_weight_q=data_args.cm_loss_weight_query,
                    cm_loss_weight_p=data_args.cm_loss_weight_document,
                )
            p_train_step = jax.pmap(
                retriever_train_step_wrapper,
                "device"
            )
        else:
            p_train_step = jax.pmap(
                retriever_train_step_naive,
                "device"
            )
